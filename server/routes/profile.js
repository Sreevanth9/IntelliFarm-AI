...
router.put(
  ...
  requireAuth,
  [
    body("name").optional({ checkFalsy: true, nullable: true }).isString().trim().isLength({ min: 2, max: 100 }).withMessage("Name must be 2-100 characters"),
    body("pincode").optional({ checkFalsy: true, nullable: true }).isString().trim().matches(/^\d{6}$/).withMessage("Pincode must be exactly 6 digits"),
    body("location").optional({ checkFalsy: true, nullable: true }).isString().trim().isLength({ min: 2, max: 200 }).withMessage("Location must be 2-200 characters"),
    body("farmSize").optional({ checkFalsy: true }).customSanitizer(val => val !== undefined && val !== null ? String(val).trim() : ""),
    body("cropsInterested").optional({ checkFalsy: true, nullable: true }).isArray(),
